---
title: 使用指南
description: Dubrify API 详细使用说明，包含所有模型的调用方法和高级功能配置
---
...
Dubrify API提供统一的API接口标准，**完全兼容OpenAI API格式**，支持无缝切换。通过一个接口地址即可访问包括OpenAI、Claude、Gemini、Midjourney等在内的所有主流AI模型。

## 基础配置

### API地址
将原始OpenAI API地址替换为Dubrify API地址：
```
原地址: https://api.openai.com
新地址: https://api.dubrify.com
```

### 身份认证
在 [ai.dubrify.com](https://ai.dubrify.com) 生成您的专属 API 密钥
...
client = OpenAI(
    ...
    api_key="sk-your-api-key"  # 替换为您的密钥
)
...
response = client.chat.completions.create(
    ...
    max_tokens=16384,
    messages=[
        {"role": "user", "content": "你好，介绍一下Dubrify平台"}
    ]
)
...
```bash
curl --request POST \
    --url https://api.dubrify.com/v1/chat/completions \
...
#### 多模态图像理解
```bash
curl https://api.dubrify.com/v1/chat/completions \
    --header 'Authorization: Bearer sk-your-api-key' \
    --header 'Content-Type: application/json' \
    --data '{
        "model": "gpt-4o",
...
### OpenAI 系列
支持所有 OpenAI 官方模型：
- `gpt-4o`, `gpt-4o-mini` — 多模态模型
- `gpt-4.1`, `gpt-4.1-mini`, `gpt-4.1-nano` — 最新一代模型
- `o3`, `o4-mini` — 推理模型

### Anthropic Claude
完全兼容 OpenAI API 格式，只需更换模型名称：
```python
response = client.chat.completions.create(
    model="claude-sonnet-4-6",  # 使用 Claude 模型
...
- `claude-haiku-4-5` — 快速响应模型

### Google Gemini
同样支持 OpenAI API 格式调用：
```python
response = client.chat.completions.create(
    model="gemini-2.5-flash",  # 使用 Gemini 模型
