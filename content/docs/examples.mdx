---
title: 代码示例
description: 各种编程语言的 Dubrify API 代码示例和最佳实践
---
...
本页面提供了使用 Dubrify API 的完整代码示例，涵盖各种常见用例和编程语言。
...
func main() {
    client := dubrify.NewClient(
        ...
        dubrify.WithBaseURL("https://api.dubrify.com/v1"),
    )
...
async def compare_models():
    ...
    models = ["gpt-4o", "gpt-4o-mini", "claude-sonnet-4-6", "gemini-2.5-flash"]
...
class DubrifyClient:
    ...
    def __init__(self, api_key: Optional[str] = None):
        ...
        if not self.api_key:
            raise ValueError("API密钥未提供，请设置API_KEY环境变量或传入api_key参数")
...
if __name__ == "__main__":
    ...
    # 简单聊天
    response = dubrify.chat("解释一下什么是深度学习")
    print("AI回复:", response)

    # 翻译
    translation = dubrify.translate("Hello, how are you today?", "中文")
    ...
    取得了突破性进展。
    """
    summary = dubrify.summarize(long_text, "简短")
    print("摘要:", summary)
```
...
<Callout type="info">
更多 API 功能和最新接口，请参考 [API Reference](/api-reference)。
</Callout>

## 更多资源

- [使用指南](/docs/guide) — API 接口和高级功能
- [视频生成](/docs/video) — Sora 和 Grok Video 视频生成
- [API Reference](/api-reference) — 交互式 API 文档
