---
title: Sora API
description: 使用 OpenAI Sora 模型生成高质量 AI 视频的完整指南，包含 API 接口、代码示例和最佳实践
---
...
使用 OpenAI Sora 模型通过 Dubrify API 生成高质量 AI 视频。

## 概述

- 文本生成视频（Text-to-Video）
- 参考图像生成视频（Image-to-Video）
- 视频 Remix 编辑
- 支持 `sora-2` 和 `sora-2-pro` 两种模型
- 多种分辨率，最高 1080p
...
**端点**: `GET https://api.dubrify.com/v1/videos/{video_id}/remix`

对已完成的视频进行风格化编辑和调整。

#### 示例请求
```bash
curl -X GET 'https://api.dubrify.com/v1/videos/video_xxxxxxxxxxxxx/remix' \
  -H 'Authorization: Bearer YOUR_API_KEY' \
...
**端点**: `GET https://api.dubrify.com/v1/videos/{video_id}`

#### 示例请求
```bash
curl -X GET 'https://api.dubrify.com/v1/videos/video_xxxxxxxxxxxxx' \
  -H 'Authorization: Bearer YOUR_API_KEY'
```
...
**端点**: `GET https://api.dubrify.com/v1/videos/{video_id}/content`
...
#### 示例请求
```bash
curl -X GET 'https://api.dubrify.com/v1/videos/video_xxxxxxxxxxxxx/content' \
  -H 'Authorization: Bearer YOUR_API_KEY' \
  -o video.mp4
```
...
async def generate_video_with_polling(
    ...
    try:
        ...
        if video.status == "completed":
            ...
            print(f"         'https://api.dubrify.com/v1/videos/{video.id}/content' \\")
